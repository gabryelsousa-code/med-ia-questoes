...
temas = {
    "🌞 Claro": {
        "bg_app": "#fdfdfd", "bg_card": "#ffffff", "bg_sidebar": "#f8f9fa",
        "text_main": "#2c3e50", "text_sec": "#666666", "border": "#e0e0e0",
        "primary": "#5E35B1", "primary_hover": "#4527a0",
        "tag_bg": "#ffeb3b", "tag_txt": "#333",
        "success_bg": "#d4edda", "error_bg": "#f8d7da"
    },
    "🌙 Escuro": {
        "bg_app": "#0e1117", "bg_card": "#262730", "bg_sidebar": "#1c1e26",
        "text_main": "#fafafa", "text_sec": "#b0b0b0", "border": "#3b3d45",
        "primary": "#9575CD", "primary_hover": "#7E57C2",
        "tag_bg": "#fbc02d", "tag_txt": "#000",
        "success_bg": "#14532d", "error_bg": "#7f1d1d"
    }
}
...
<style>
    ...
    /* Textos */
    h1, h2, h3, p, div, span, label {{ color: {paleta['text_main']} !important; }}
...
if not st.session_state.user:
    ...
    with c2:
        st.markdown(f"<br><br><h1 style='text-align: center; color: {paleta['primary']} !important;'>🦉 MedResidency</h1>", unsafe_allow_html=True)
...
if pagina == "🔍 Navegador":
    ...
    # Filtro de Tags em Python (para performance)
    if f_tags and lista:
        ...
        if st.session_state.questoes_carregadas:
            ...
            <div class="questao-card">
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <div style="color:{paleta['primary']}; font-weight:bold;">
                        {q['disciplina']} | {q.get('assunto')}
                    </div>
                    <div class="texto-secundario">ID: {q.get('id_original') or q['id']} • {idx+1}/{len(qs)}</div>
                </div>
                <div style="margin-bottom:15px;">{t_html}</div>
                <div style="font-size:1.1rem; line-height:1.6;">{q['enunciado']}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.session_state.resposta_mostrada or ja_resp:
                ...
                st.markdown(f"<div style='background:{bg}; padding:15px; border-radius:8px; color:#333; font-weight:bold; margin-top:15px;'>{msg}</div>", unsafe_allow_html=True)
